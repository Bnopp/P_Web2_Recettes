...
class HomeController extends Controller
{
	...
	private function contactAction()
	{
		...
		if (isset($_GET['send']) && $_GET['send'] == 1)
		{
			...
			if (preg_match("/^\p{L}{1,}$/u", $_POST['name']) && preg_match("/^([A-z]{1,}\.{0,1}){1,}@[A-z]{1,}\.{1}[A-z]{1,}$/u", $_POST['email']) && preg_match("/^\p{L}{1,}$/u", $_POST['subject']) && preg_match("/^(\p{L}|[0-9]|\W){1,}$/u", $_POST['message']))
			{
	...
	/**
	 * It checks if the user is connected and if the user is trying to connect or
	 * disconnect, if he's trying to connect, it checks if the username and password are valid, if they
	 * are, it connects the user, if not, it displays an error message.
	 *
	 * @return mixed content of the view.
	 */
	private function connectAction()
	{
		...
		if (isset($_GET['connect']))
		{
			...
			{
				if (preg_match("/^\p{L}{1,}$/u", $_POST['username']) && preg_match("/^(\p{L}|[0-9]|\W){1,}$/u", $_POST['password']))
				{
				...
				{
					$_SESSION['error'] = "Le format que vous avez utilisé pour entrer vos données n'est pas valide, veuillez vérifier l'écriture de votre nom d'utilisatuer et mot de passe.";
					header("Location: index.php?controller=home&action=error");
					die();
				}
			}
